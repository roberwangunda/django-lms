...
def semester_list_view(request):
    terms = Semester.objects.all().order_by('-is_current_term', '-term')
    return render(request, 'app/semester_list.html', {"terms": terms, })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if data == 'True':
                ...
                try:
                    if Semester.objects.get(term=term, session=ss):
                        messages.error(request, term + " term in " + session.session + " session already exist")
                    ...
                    if terms:
                        for term in terms:
                            if term.is_current_term == True:
                                unset_term = Semester.objects.get(is_current_term=True)
...
def semester_update_view(request, pk):
    term = Semester.objects.get(pk=pk)
    if request.method == 'POST':
        if request.POST.get('is_current_term') == 'True': # returns string of 'True' if the user selected yes for 'is current semester'
            unset_term = Semester.objects.get(is_current_term=True)
            unset_term.is_current_term = False
            unset_term.save()
            unset_session = Session.objects.get(is_current_session=True)
            ...
            unset_session.save()
            new_session = request.POST.get('session')
            form = SemesterForm(request.POST, instance=term)
            if form.is_valid():
                set_session = Session.objects.get(pk=new_session)
                set_session.is_current_session = True
                set_session.save()
        ...
        form = SemesterForm(instance=term)
    return render(request, 'app/semester_update.html', {'form': form})
...
def semester_delete_view(request, pk):
    term = get_object_or_404(Semester, pk=pk)
