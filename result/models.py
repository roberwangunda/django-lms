...
class TakenCourse(models.Model):
    ...
    def calculate_gpa(self, total_credit_in_semester):
        ...
        student = TakenCourse.objects.filter(student=self.student, course__level=self.student.level, course__term=current_term)
        p = 0
        point = 0
        for i in student:
    ...
    def calculate_cgpa(self):
        ...
        if str(current_term) == SECOND:
            ...
            sec_sem_gpa = 0.0
            try:
                first_sem_result = Result.objects.get(student=self.student.id, term=FIRST, level=self.student.level)
                first_sem_gpa += first_sem_result.gpa
            except:
                first_sem_gpa = 0

            try:
                sec_sem_result = Result.objects.get(student=self.student.id, term=SECOND, level=self.student.level)
