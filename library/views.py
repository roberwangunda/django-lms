...
from library.models import Books, Students 
from django.contrib.auth import authenticate, login, logout, update_session_auth_hash
from django.contrib.auth.decorators import login_required
from django.urls import reverse_lazy
from .forms import BooksItemFormset
...
@login_required
#getting profile for a specific student
def view_borrow_profile(request , pk =None):
    context =context_data(request)
    context['page'] = 'view_student'
    context['page_title'] = 'View Student'
    if pk is None:
...
class BooksCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(BooksCreateView, self).get_context_data(**kwargs)
        books = Books.objects.get(pk=self.request.GET["books"])
        context["books"] = books
        return context
...
#     fields = ["title", "isbn", "publisher"]
#     success_url = reverse_lazy("form_snippet")

#     def get_context_data(self, **kwargs):
#         context = super(BooksCreateView, self).get_context_data(**kwargs)
#         context["fields"] = Books.objects.filter(student=self.object)
#         return context
