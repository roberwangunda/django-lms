...
from .decorators import lecturer_required, librarian_required, student_required, admin_required
from course.models import Course
from result.models import TakenCourse
from app.models import Session, Semester
from .forms import StaffAddForm, StudentAddForm, ProfileUpdateForm, ParentAddForm, LibrarianAddForm
...
def profile(request):
    ...
    try:
        ...
        current_term = get_object_or_404(Semester, is_current_term=True, session=current_session)
...
def profile_single(request, id):
    ...
    if user.is_lecturer:
        courses = Course.objects.filter(allocated_course__lecturer__pk=id).filter(term=current_term)
        context = {
...
@login_required
@admin_required
def librarian_add_view(request):
    if request.method == 'POST':
        form = LibrarianAddForm(request.POST)
        first_name = request.POST.get('first_name')
        last_name = request.POST.get('last_name')
        if form.is_valid():
            form.save()
            messages.success(request, "Account for librarian " + first_name + ' ' + last_name + " has been created.")
            return redirect("librarian_list")
    else:
        form = LibrarianAddForm()

    context = {
        'title': 'Librarian Add | DjangoSMS',
        'form': form,
    }

    return render(request, 'accounts/add_librarian.html', context)


@login_required
@admin_required
def edit_librarian(request, pk):
    instance = get_object_or_404(User, is_librarian=True, pk=pk)
    if request.method == 'POST':
        form = ProfileUpdateForm(request.POST, request.FILES, instance=instance)
        full_name = instance.get_full_name
        if form.is_valid():
            ...
            messages.success(request, 'Librarian ' + full_name + ' has been updated.')
            return redirect('librarian_list')
        else:
            messages.error(request, 'Please correct the error below.')
    else:
        form = ProfileUpdateForm(instance=instance)
    return render(request, 'accounts/edit_librarian.html', {
        'title': 'Edit Librarian | DjangoSMS',
        'form': form,
    })
...
class LibrarianListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = "Librarians | DjangoSMS"
        return context
...
def delete_librarian(request, pk):
    ...
    full_name = librarian.get_full_name
    librarian.delete()
    messages.success(request, 'Librarian ' + full_name + ' has been deleted.')
    return redirect('librarian_list')
...
class StudentListView(ListView):
    ...
    # paginate_by = 50000  # if pagination is desired

    def get_queryset(self):
        queryset = Student.objects.all()
        query = self.request.GET.get('student_id')
        if query is not None:
            queryset = queryset.filter(Q(level=query))
        return queryset
